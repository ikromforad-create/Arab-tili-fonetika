...
begin
  ...
  if found then
    ...
    values (normalized_username, trim(p_first_name), trim(p_last_name), crypt(p_password, gen_salt('bf')))
    returning * into profile_row;
  end if;
...
begin
  ...
  if profile_row.password_hash is null then
    ...
    where p.id = profile_row.id
    returning * into profile_row;
  elsif profile_row.password_hash <> crypt(p_password, profile_row.password_hash) then
    raise exception 'Login yoki parol noto''g''ri.';
  end if;
...
grant execute on function public.profile_app_payload(uuid) to anon, authenticated;
grant execute on function public.register_profile(text, text, text, text) to anon, authenticated;
grant execute on function public.login_profile(text, text) to anon, authenticated;
grant execute on function public.save_section_result(uuid, integer, text, integer, integer, integer) to anon, authenticated;
grant execute on function public.update_profile_avatar(uuid, text) to anon, authenticated;
